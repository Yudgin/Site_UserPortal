...
//   80671234567    → 380671234567  (легаси-формат «8 0XX…»)
//   00380671234567 → 380671234567  (международный префикс 00)
//   +380 67 123 45 67 → 380671234567
export const normalizePhone = (raw: string | null | undefined): string => {
  let d = (raw || '').replace(/\D/g, '')
  if (!d) return ''
  if (d.startsWith('00')) d = d.slice(2) // международный префикс 00 → убираем
  if (d.length === 11 && d.startsWith('80')) d = '380' + d.slice(2) // легаси «8 0XX XXX XX XX»
  else if (d.length === 10 && d.startsWith('0')) d = '38' + d // 0XX XXX XX XX
  else if (d.length === 9) d = '380' + d // XX XXX XX XX без ведущего 0
  return d
}
